...
#include "Matrix.cpp" //Класс работы с вектором
	...
	while (k != 'q')
	{
		...
		switch (k)
		{
			...
			{	//Сортировка методом "Пузырька"
				v.sort(MyMatrix<int>::Bubble);
				std::cout<<"Обменов: " << v.nswap << std::endl;
				break;
			}
			case '4':
			{	// Сортировка методом вставок
				v.sort(MyMatrix<int>::Insertion);
				std::cout<<"Обменов: " << v.nswap << std::endl;
				break;
			}
			case '5':
			{	//Сортировка подсчётом
				v.sort(MyMatrix<int>::Count);
			...
			{	//Бинарный поиск элемента вектора
				...
				start = clock();
				//
				result = v.find(MyMatrix<int>::Binary, t);
				if (result < 0)
				{
			...
			{	//Интерполяционный поиск элемента вектора
				...
				start = clock();
				//
				result = v.find(MyMatrix<int>::Interpolation, t);
				if (result < 0)
				{
					std::cout << "Элемент не найден\n";
				} else {
